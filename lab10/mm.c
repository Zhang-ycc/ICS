 ...
 * mm.c
 * 
 * In this approach, an inplicit free list is used to organize blocks.
 * Each block has a header and a footer with 4 bytes each, containing
 * block size and allocation tag. The allocator is aligned to 8, using
 * LIFO-ordering, next-fit and immediate coalescing.
 *
 * 520021910266 Zhang Yuechen
 */
#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
...
static char* prev_listp = 0;  /* Pointer to previous block */
...
static void* coalesce(void* bp)
{
    ...
    if ((prev_listp > (char*)bp) && (prev_listp < NEXT_BLKP(bp)))
        prev_listp = bp;

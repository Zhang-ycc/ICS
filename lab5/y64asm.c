...
parse_t parse_digit(char **ptr, long *value)
{
	...
	//need unsigned long, use stroull
	char* end;
...
parse_t parse_mem(char **ptr, long *value, regid_t *regid)
{
	...
	if (IS_DIGIT(*ptr)){
		if (parse_digit(ptr, value) < 0)
			return PARSE_ERR;
	}
	if (parse_delim(ptr, '(')){
		if (parse_reg(ptr, regid) < 0)
			return PARSE_ERR;
		if (parse_delim(ptr, ')') < 0)
			return PARSE_ERR;
		return PARSE_MEM;
	}
...
parse_t parse_data(char **ptr, char **name, long *value)
{
    ...
    /* if IS_LETTER, then parse the symbol */
	if (IS_LETTER(*ptr)){
		parse_t p =  parse_symbol(ptr, name);
		//parse_delim(ptr,':');
		return p;
	}
...
*           call SUM  #invoke SUM function */
	...
	if (IS_LETTER(str)||(*str) == '.'){
		if (parse_instr(&str, &inst) <  0){
		//printf("%c%c\n",*str, *(str+16));
			if (parse_label(&str, &name) != -1){
			...
			else if (parse_instr(&str, &inst) < 0){
				line->type = TYPE_ERR;
				return line->type;
			}
		}
	...
	switch (HIGH(inst->code)){
		...
		case I_RRMOVQ: case I_ALU:
			if (parse_reg(&str, &regA) < 0){
				line->type = TYPE_ERR;
				return line->type;
			}
			if (parse_delim(&str,',') < 0){
				err_print("Invalid ','");
				line->type = TYPE_ERR;
				return line->type;
			}
			if (parse_reg(&str, &regB) < 0){
				line->type = TYPE_ERR;
				return line->type;
			}
	...
	switch (LOW(inst->code)){
		case D_DATA:{
			parse_t p  = parse_data(&str, &name, &value);
			if (p < 0){
				line->type = TYPE_ERR;
				return line->type;
			}
...
int assemble(FILE *in)
{
	...
	type_t t = parse_line(line);
		...
		if (bin.bytes){
			if (bin.addr + bin.bytes > max_byte)
				max_byte = bin.addr + bin.bytes;
				}
	}
...
int relocate(void)
{
    ...
    while (rtmp) {
        ...
        /* next */
	    for (int i = 0; i < 8; i++)
		    rtmp->y64bin->codes[i + bias] = (s->addr >> (i*8)) & 0xFF; 
        rtmp = rtmp->next;
    }
    return 0;
}

...
bool_t cond_doit(cc_t cc, cond_t cond) 
{
    ...
    if (cond == 0)
	    flag = 1;
    if (cond == 1)
	    flag = (GET_SF(cc)^GET_OF(cc))|GET_ZF(cc);
    if (cond == 2)
	    flag = GET_SF(cc)^GET_OF(cc);
    if (cond == 3)
	    flag = GET_ZF(cc);
    if (cond == 4)
	    flag = !GET_ZF(cc);
...
stat_t nexti(y64sim_t *sim)
{
	...
	//get_long_val(sim->m, next_pc, &dest);
    }
	...
	cond_t c = ifun;
	bool_t flag = cond_doit(sim->cc, c);
	if (flag == TRUE){
		long_t tmp;
		tmp = get_reg_val(sim->r, regA);
	...
	set_reg_val(sim->r, regB, dest);
	sim->pc = next_pc + 8;
	break;
      case I_RMMOVQ: {/* 4:0 regA:regB imm */
	long_t tmp = get_reg_val(sim->r, regA);
	long_t addr = regB + dest;
	set_long_val(sim->m, addr, tmp);
	sim->pc = next_pc + 8;
	break;
		     }
      case I_MRMOVQ: {/* 5:0 regB:regA imm */
	long_t addr = get_reg_val(sim->r, regB) + dest;
	long_t tmp;
	if (!get_long_val(sim->m, addr, &tmp)){
	...
	long_t a = get_reg_val(sim->r,regA);
	long_t b = get_reg_val(sim->r,regB);
	long_t val = compute_alu(ifun, a, b);
	sim->cc = compute_cc(ifun, a, b, val);
	...
	long_t addr = rsp + 8;
	long_t ret_addr;
	if (!get_long_val(sim->m, rsp, &ret_addr)){
		err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, rsp);
	       	return STAT_ADR;
	}
	set_reg_val(sim->r, 4, addr);
	sim->pc = ret_addr;
	break;
			   }
      case I_PUSHQ: /* A:0 regA:F */{
	long_t tmp = get_reg_val(sim->r, regA);
	long_t addr = get_reg_val(sim->r, 4) - 8;
	set_reg_val(sim->r, REG_RSP, addr);
	if (!set_long_val(sim->m, addr, tmp)){
		err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, addr);
	       	return STAT_ADR;
	}
	sim->pc = next_pc;
